...
class ReportController extends ControllerBase {
  ...
  public function overview() {
    ...
    if (!empty($info['maxmemory'])) {
      $memory_value = $this->t('@used_memory / @max_memory (@used_percentage%), maxmemory policy: @policy', [
        ...
        '@used_percentage' => (int) (($info['used_memory'] ?? $info['Memory']['used_memory']) / $info['maxmemory'] * 100),
    ...
    // Warnings/hints.
    if (!empty($info['maxmemory_policy']) && $info['maxmemory_policy'] == 'noeviction') {
   ...
   *
   * @todo remove and start using ByteSizeMarkup when D9 is unsupported.
   */
