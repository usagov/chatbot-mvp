...
class S3fsService implements S3fsServiceInterface {
   ...
   *   The s3fs logger.
   */
  public function __construct(Connection $connection, ConfigFactoryInterface $config_factory, TimeInterface $time, ModuleHandlerInterface $module_handler, LoggerInterface $logger = NULL) {
  ...
  public function validate(array $config) {
    ...
    try {
      ...
      $this->moduleHandler->alter('s3fs_upload_params', $putOptions);

      $s3->putObject($putOptions);

      $options = ['Bucket' => $config['bucket'], 'Key' => $key];
      $this->moduleHandler->alter('s3fs_command_params', $options);

      $object = $s3->getObject($options);
      if ($object) {
        $successPut = TRUE;
    ...
    if (empty($config['read_only']) && !Settings::get('s3fs.upload_as_private')) {
      try {
        ...
        $this->moduleHandler->alter('s3fs_command_params', $options);
        if ($object = $s3->getObject($options)) {
          $s3->deleteObject($options);
        }
      }
      catch (S3Exception $e) {
