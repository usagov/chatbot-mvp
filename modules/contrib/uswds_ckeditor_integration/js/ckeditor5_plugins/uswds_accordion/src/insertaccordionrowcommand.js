...
export default class InsertAccordionRowCommand extends Command {
  ...
  execute() {
    ...
    selection
      ...
      .forEach((ancestor) => {
        if (
          ancestor.name === 'accordionContent' ||
          ancestor.name === 'accordionTitle'
        ) {
          commandEl = ancestor;
        }
      });
      ...
      editor.model.change((writer) => {
        ...
        if (this.order === 'below') {
          let insertAfterIndex =
            commandEl.name === 'accordionContent'
              ? commandEl.index
              : commandEl.index + 1;
          ...
          position = writer.createPositionAfter(
            commandEl.parent.getChild(insertAfterIndex),
          );
        }
        else {
          ...
          position = writer.createPositionBefore(
            commandEl.parent.getChild(insertBeforeIndex),
          );
        }
