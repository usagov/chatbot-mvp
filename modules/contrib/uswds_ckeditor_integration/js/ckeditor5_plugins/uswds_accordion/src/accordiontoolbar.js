...
export default class AccordionToolbar extends Plugin {
  ...
  afterInit() {
    ...
    const accordionContentToolbarItems = editor.config.get(
      'uswdsAccordionConfig.contentToolbar',
    );

    const accordionToolbarItems = editor.config.get(
      'uswdsAccordionConfig.tableToolbar',
    );

    if (accordionContentToolbarItems) {
      widgetToolbarRepository.register('accordionContent', {
        ariaLabel: t('USWDS Accordion toolbar'),
